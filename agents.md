# class-tools-2（ClassHub）（專案藍圖）

> 本檔為跨 Agent 通用的專案藍圖（AGENTS.md 開放標準）。任何 Agent 的每個 session 都應先讀本檔＋`handoff.md`。

## 專案簡介

課堂教學多功能工具箱 ClassHub，包含抽籤、噪音監測、隨機分組、座位表、計時器與團隊計分板。純前端靜態單頁應用程式（SPA），技術棧為 HTML5、Vanilla CSS、Vanilla JavaScript、Vite，另使用 Canvas Confetti 與 Font Awesome。
