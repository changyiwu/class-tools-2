...
- **用瀏覽器預覽驗證 canvas／`requestAnimationFrame` 動畫時，rAF 會被節流**：光是等待不會推進畫格，要連續截圖逼出重繪，或直接呼叫收尾函式驗結果
...
- **抽籤支援「一次抽 N 位」，且 N 位都要是抽出來的**：轉盤格子排列用亂數打散，停下後指針**逐格往前掃**、每位停留一下（常數 `SWEEP_STEP_MS`／`SWEEP_HOLD_MS`）；拉霸把中籤者排在滾動條尾端依序停留；神秘翻牌要翻滿 N 張才成局。不要退回「動畫抽一位、其餘隨機補齊」——老師看得出來後面幾位不是抽出來的
- **浮層裡再包一層 flex 容器時，容器要給 `width: 100%`**：否則容器是 shrink-to-fit，子項的 `max-width: 92%` 會換算成「自己內容寬度的 92%」，連兩個字的名字都被 `text-overflow` 切成「…」
